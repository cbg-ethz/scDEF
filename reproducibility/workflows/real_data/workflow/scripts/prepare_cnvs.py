...
# Select donors
sub_adata = adata[adata.obs.query(f"donor_id == '{donor_1}' or donor_id == '{donor_2}'").index]

# Select samples
sub_adata = sub_adata[sub_adata.obs.query("author_tumor_subsite == 'Left Adnexa' or author_tumor_subsite == 'Left Ovary'").index]
...
sub_adata.var = sub_adata.var.rename(columns={'start_position': 'start', 'end_position': 'end', 'chromosome_name': 'chromosome'})
...
sub_adata.var['mt'] = sub_adata.var_names.str.startswith('MTRN')  # annotate the group of mitochondrial genes as 'mt'
sc.pp.calculate_qc_metrics(sub_adata, qc_vars=['mt'], percent_top=None, log1p=False, inplace=True)
...
sc.pp.highly_variable_genes(sub_adata, flavor='seurat_v3', n_top_genes=5000, layer='counts',
                          batch_key='donor_id') # Not required, but makes scDEF faster
...
sub_adata.var['chromosome'] = sub_adata.var['chromosome'].cat.rename_categories(dict(zip([str(i) for i in range(23)] + ['X', 'Y'], [f'chr{i}' for i in range(23)] + ['chrX', 'chrY'])))
...
cnv.tl.infercnv(
    ...
    reference_cat=[l for l in sub_adata.obs['author_cell_type'].unique() if 'cancer' not in l],
    window_size=250,
    calculate_gene_values=True, 
)
cnv.pl.chromosome_heatmap(sub_adata, groupby="author_cell_type", save=snakemake.output.cnv_full_fname)
...
sub_sub_adata = init_data[init_data.obs.query(f"(donor_id == '{donor_1}' and author_cell_type == '{celltype_1}') \
                            or (donor_id == '{donor_2}' and author_cell_type == '{celltype_2}') ").index]

cnv.pl.chromosome_heatmap(sub_sub_adata, groupby="author_cell_type", save=snakemake.output.cnv_subset_fname)
...
sc.pp.highly_variable_genes(sub_sub_adata, flavor='seurat_v3', 
                            n_top_genes=snakemake.params.n_top_genes, layer='counts',
                           batch_key='donor_id') # Not required, but makes scDEF faster
