...
adata.uns["clusters_colors"] = [
    colors[clus] for clus in adata.obs["clusters"].cat.categories
]
...
adata.obs["clusters_neoblasts"] = (
    adata.obs["clusters"].map(map_neoblasts).astype("category")
)
...
adata.obs["clusters_coarse"] = adata.obs["clusters"].map(map_coarse).astype("category")

adata.uns["clusters_coarse_colors"] = [
    colors_coarse[c] for c in adata.obs["clusters_coarse"].cat.categories
]

sc.pl.tsne(
    adata,
    color=["clusters", "clusters_neoblasts", "clusters_coarse"],
    legend_loc="on data",
    legend_fontsize=5,
)
