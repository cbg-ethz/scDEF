...
# sc.pl.umap(sub_adata, color=[ "author_cell_type", "donor_id"], frameon=False)
...
sub_sub_adata.obs["Batch"] = sub_sub_adata.obs["donor_id"]
sub_sub_adata.uns["true_markers"] = dict()
sub_sub_adata.uns["hierarchy_obs"] = ["Cell types", "Donor"]
for i, obs in enumerate(sub_sub_adata.uns["hierarchy_obs"]):
    sub_sub_adata.obs[obs] = "h" * i + sub_sub_adata.obs[obs].astype(str)
...
sc.pl.umap(
    sub_sub_adata, color=sub_sub_adata.uns["hierarchy_obs"] + ["Batch"], frameon=False
)

# Write new h5ad file
sub_sub_adata.write(snakemake.output.adata_subset_fname)
