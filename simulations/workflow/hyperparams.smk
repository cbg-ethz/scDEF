...
rule gather_tau_mu_scores:
    ...
    run:
        import pandas as pd
        methods_files = dict(zip(["tau", "mu"], [snakemake.input['tau_fname_list'], snakemake.input['mu_fname_list']]))
        for method in methods_files:
            ...
            if method == "tau":
                scores.to_csv(snakemake.output['tau_output'], index=False)   
            else:
                scores.to_csv(snakemake.output['mu_output'], index=False)   
