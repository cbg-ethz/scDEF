    ...
    def __init__(
        ...
        **kwargs
        ...
        if markers_layer == 0:
            for layer in range(self.n_layers):
                if layer == 0:
                    ...
                    size = int(np.ceil(self.n_markers/(self.n_factors_per_set*layer)))
                    name = "h"*layer
                    if size <= 1:
                        break
        ...
        super(iscDEF, self).__init__(
            adata, layer_sizes=layer_sizes, layer_shapes=.3, use_brd=False, **kwargs
        )
        ...
        if self.markers_layer != 0:
            ...
            for layer_idx in range(1, self.n_layers):
                ...
                self.w_priors[layer_idx][1] = jnp.array(np.ones(connectivity_matrix.shape))
        ...
        for i, cellgroup in enumerate(self.marker_names):
            ...
            for gene in self.markers_dict[cellgroup]:
                ...
                if len(loc) == 0:
                    self.logger.warning(f"Did not find gene {gene} for set {cellgroup} in AnnData object.")
                self.marker_gene_locs.append(loc)
                self.gene_sets[factors_start:factors_end, loc] = self.gs_big_scale

        self.w_priors[0][0] = jnp.array(self.gene_set_strength * self.gene_sets) # shape
        self.w_priors[0][1] = jnp.array(self.gene_set_strength * np.ones(self.gene_sets.shape)) # rate
        ...
        for idx in range(self.n_layers):
            ...
            if self.markers_layer == 0:
                if idx == 0:
                    ...
                    self.factor_names.append([f"{self.layer_names[idx]}{str(i)}" for i in range(len(self.factor_lists[idx]))])
            else: # if not zero, it's the top one, so append gene set names to each subsequent layer
                if idx == self.n_layers-1:
                    factor_names = [marker for i, marker in enumerate(self.marker_names) if i in self.factor_lists[idx]]
                else:
                    ...
                    for marker_idx, marker_name in enumerate(self.marker_names):
                        ...
                        filtered_sub_factors = [factor for factor in sub_factors if factor*marker_idx in self.factor_lists[idx]]
                        for sub_factor in range(len(filtered_sub_factors)):
                            marker_factor_names.append(marker_name + f'_{self.layer_names[idx]}{sub_factor}')
