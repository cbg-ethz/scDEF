...
# Run scHPF
best, cscores, gscores = scdef.other_methods.run_schpf(X, range(k_min,k_max))
assignments = np.argmax(cscores,axis=1)

# Compute clustering scores
asw = silhouette_score(cscores, adata.obs['Group'])
ari = adjusted_rand_score(adata.obs['Group'], assignments)
...
for group in np.unique(adata.obs['Group']):
    ...
    # Compute ROC AUC
    aucs.append(roc_auc_score(true_markers, gscores[:,factor]))
...
for i in range(chc_reps):
    ...
    X = scipy.sparse.coo_matrix(adata[train_set].X)
    best, cscores, gscores = scdef.other_methods.run_schpf(X, range(k_min,k_max))
    gene_rankings = []
    for k in range(cscores.shape[1]):
        gene_rankings.append(adata.var_names[np.argsort(gscores[:,k])][:50])
