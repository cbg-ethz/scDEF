...
ard = snakemake.params["ard"]
n_factors = snakemake.params["n_factors"]
n_hfactors = snakemake.params["n_hfactors"]
...
# Run scDEF
scd = scdef.scDEF(adata, n_factors=n_factors, n_hfactors=n_hfactors, shape=shape)
scd.learn(n_epoch_schedule=[n_iters]*3)
tokeep = scd.filter_factors(annotate=True, ard=[ard, ard])
tokeep = scd.filter_factors(annotate=False, ard=[ard, ard])
gene_scores = scd.pmeans['W'][tokeep[0]] * scd.pmeans['gene_scale']

# Compute clustering scores
asw = silhouette_score(scd.pmeans['z'][:,tokeep[0]] * scd.pmeans['cell_scale'], adata.obs['Group'])
...
for group in np.unique(adata.obs['Group']):
    ...
    factors, counts = np.unique(adata.obs['X_factor'][cells], return_counts=True)
...
for i in range(chc_reps):
    ...
    scd = scdef.scDEF(adata[train_set], n_factors=n_factors, n_hfactors=n_hfactors, shape=shape)
    scd.learn(n_epoch_schedule=[n_iters]*3)
    tokeep = scd.filter_factors(annotate=False, ard=[ard, ard])
    gene_scores = scd.pmeans['W'][tokeep[0]] * scd.pmeans['gene_scale']
    gene_rankings = []
